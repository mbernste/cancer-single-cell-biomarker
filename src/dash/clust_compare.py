...
df = load_data.load_gsva_compare_cluster()
...
for clust in df.index:
    if 'PJ0' in clust:
        cat_to_true['glioma'].append(clust)
    elif 'LX' in clust:
        cat_to_true['lung adenocarcinoma'].append(clust)
    elif 'GSE146026' in clust:
        cat_to_true['ovarian cancer'].append(clust)
    elif 'GSE72056' in clust:
        cat_to_true['melanoma'].append(clust)
